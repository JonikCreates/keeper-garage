...
type Snapshot = {
  ...
  schedule: Array<{
    ...
    lastService: null | { serviceDate: string; mileage: number };
  }>;
};
...
export default function VehicleItemSnapshot({ slug }: { slug: string }) {
  ...
  if (!data?.vehicle || !item) {
    return <div className="snapshot-loading">Loading your vehicle context…</div>;
  }
  ...
  return (
    <section className="vehicle-snapshot">
      ...
      <Link href="/#schedule" className="button button-primary">Record or review service</Link>
    </section>
  );
}
